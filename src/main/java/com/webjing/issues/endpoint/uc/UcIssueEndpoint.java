...
public class UcIssueEndpoint implements CustomEndpoint {
    ...
    private final IssueService issueMessageService;
    ...
    private Mono<ServerResponse> updateMyIssue(ServerRequest request) {
        ...
        return getMyIssueDetail(name)
            .flatMap(oldIssueMessage -> {
                Issue.IssueMessageSpec oldSpec = oldIssueMessage.getSpec();

                return request.bodyToMono(Issue.class)
                    .doOnNext(newMoment -> {
                        Issue.IssueMessageSpec newSpec = newMoment.getSpec();
    ...
    private Mono<ServerResponse> listMyMoment(ServerRequest request) {
        return getCurrentUser()
            ...
            .flatMap(issueMessageService::listIssue)
