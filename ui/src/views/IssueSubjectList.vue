...
import type {Issue, IssueSubject, IssueSubjectSpecSubjectTypeEnum, ListedIssueSubject} from "@/api/generated";
...
const { issueSubjects, isLoading, isFetching, refetch, total } = useIssueSubjectListFetch(
  page,
  size,
  keyword,
  selectedSort,
...
const checkSelection = (listedIssueSubject: ListedIssueSubject) => {
  return (
    listedIssueSubject.issueSubject.metadata.name === selectedIssueSubject.value?.metadata.name ||
    selectedIssueSubjectNames.value.includes(listedIssueSubject.issueSubject.metadata.name)
  );
};
      ...
      <div class="w-full">
        <VCard :body-class="['!p-0']">
              ...
              >
                ...
                <div class="w-auto sm:w-auto">
                  <VSpace spacing="sm" class="flex flex-wrap">
                    ...
                    <FilterDropdown :items="subjectTypeOptions" v-model="selectedSubjectType" label="依托主体类型" />
          ...
          <Transition v-else-if="!issueSubjects?.length" appear name="fade">
            <VEmpty message="你可以尝试刷新或者新建issue依托主体" title="当前没有任何Issue依托主体">
              <template #actions>
                <VSpace>
                  <VButton @click="refetch"> 刷新</VButton>
                  <VButton v-permission="['plugin:issueSubject:manage']" type="primary" @click="handlerNewIssue">
                    <template #icon>
          ...
          <Transition v-else appear name="fade">
            <ul class="box-border px-3 h-auto w-full divide-y divide-gray-100 grid xl:grid-cols-2 2xl:grid-cols-3 grid-cols-1 gap-3" role="list">
              <li v-for="listedIssueSubject in issueSubjects" :key="listedIssueSubject.issueSubject.metadata.name">
                <IssueSubjectItem
