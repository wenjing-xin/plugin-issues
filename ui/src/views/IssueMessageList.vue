...
import { useIssueMessageListFetch } from "@/composables/use-consoleIssueMessage";
...
import { issueMessageApiClient, issueTemplateApiClient } from "@/api";
...
const updateIssueMessage = ref<IssueMessage>();
...
const selectedIssueMessage = ref<IssueMessage>();
...
const handleCheckAllChange = (e: Event) => {
  ...
  if (checkedAll.value) {
    selectedIssueMessageNames.value =
      issueMessages.value?.map((listedIssueMessage: ListedIssueMessage) => {
...
const checkSelection = (listedIssueMessage: ListedIssueMessage) => {
    ...
    onConfirm: async () => {
      try {
        const promises = selectedIssueMessageNames.value.map((name: string) => {
          return issueMessageApiClient.issueMessage.patchIssueMessage({
            name: name,
            jsonPatchInner: [
              {
                op: "add",
...
const handleDeleteInBatch = async () => {
  Dialog.warning({
    ...
    onConfirm: async () => {
      try {
        const promises = selectedIssueMessageNames.value.map((name: string) => {
          return issueMessageApiClient.issueMessage.deleteIssueMessage({
            name: name,
          });
        });
        await Promise.all(promises);
...
//更新issue
const handlerUpdateIssue = (issue: IssueMessage) => {
