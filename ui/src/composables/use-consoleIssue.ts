...
import { consoleIssueApiClient } from "@/api";
import type { ListedIssue } from "@/api/generated";
export function useIssueMessageListFetch(
  ...
  const {
    ...
    queryFn: async () => {
      const { data } = await consoleIssueApiClient.issue.listIssueMessages({
        page: page.value,
        size: size.value,
    ...
    refetchInterval: (data) => {
      const hasDeletingData = data?.some((item: ListedIssue) => !!item.issueMessage.metadata?.deletionTimestamp);
